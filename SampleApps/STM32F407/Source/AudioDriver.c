...
void I2S3_DMA_Complete_CallBack(void)
{
    ...
    else
    {
        ...
        // awe_audioGetPumpMask will skip a pump if a pump is still in progress. 
        // This represents an overflow condition.
        ...
        if (layoutMask > 0)
        {
            ...
            if (layoutMask & 1)
            {
                UINT32 nLayoutFrameSize;

                // Get the layout audio frame size in samples
                awe_layoutGetInputBlockSize(&g_AWEInstance, 0U, &nLayoutFrameSize);

                // If layout frame size equals DMA frame size call audio pump directly
                if (nLayoutFrameSize == AUDIO_BLOCK_SIZE)
                {
                    HighPriorityPump();
                }
                else
                {
                    // Signal a lower priority pump to begin when this interrupt handler exits
